...
function RepositorioList() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                const data = await multimediaService.fetchRepositorio();
                setItems(data);
            } catch (err) {
    ...
    const handleEditar = (item) => {
        navigate('/admin/repositorio', { state: { item } });
    };

    if (loading) return <div className="admin-container"><p>Cargando repositorio...</p></div>;
    if (error) return <div className="admin-container"><p>Error: {error}</p></div>;

    return (
        <div className="admin-container">
            <h2>Listado de Repositorio</h2>
            {items.length === 0 && <p>No hay items en el repositorio.</p>}
